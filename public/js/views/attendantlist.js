window.AttendantListView = Backbone.View.extend({
    ...
    render: function () {
        ...
        $(this.el).html(new AttendantsView().render().el);

        if (len > 0) {
            for (var i = startPos; i < endPos; i++) {


                jsonObject = attendants[i].toJSON();



                html += '<tr>';
                html += '<td>' + jsonObject.registration + '</td>';
                html += '<td><a href="#attendants/' + jsonObject._id + '">' + jsonObject.name + '</td>';
            ...
            $('#tbAttendants > tbody:last', this.el).append(html);
            $(this.el).append(new AttendantsPaginator({ model: this.model, page: this.options.page }).render().el);
        }
        return this;
    }
});

window.AttendantsView = Backbone.View.extend({
...
window.AttendantsPaginator = Backbone.View.extend({
